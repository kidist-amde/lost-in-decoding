...
def _format_metric_results(res):
    """Format a metric result dict as a pretty aligned string."""
    if isinstance(res, dict):
        lines = []
        for k, v in sorted(res.items()):
            if isinstance(v, float):
                lines.append(f"  {k:<30s} {v:.5f}")
            else:
...
def load_and_evaluate(qrel_file_path, run_file_path, metric):
    ...
    if metric == "mrr_10":
        ...
        print(f"  {'MRR@10':<30s} {res:.5f}")
        return {"mrr_10": res}
    else:
        res = evaluate(run, qrel, metric=metric)
        print(_format_metric_results(res))
        return res
