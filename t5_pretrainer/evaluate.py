...
def evaluate(args):
    ...
    # print evaluation header with config info
    # only show topk / beam size when they were explicitly passed on the
    # command line so we never print misleading default values.
    ...
    if any(a.startswith("--topk") for a in cli_args):
        print(f"  Top-K:      {args.topk}")
    if any(a.startswith("--max_new_token_for_docid") for a in cli_args):
        print(f"  Beam size (max_new_token_for_docid): {args.max_new_token_for_docid}")
    print(sep)
    ...
    if res_all_datasets:
        all_metrics = []
        for ds_res in res_all_datasets.values():
            for k in ds_res:
        ...
        for m in all_metrics:
            header += f" {m:>15s}"
        print(header)
        print(f"  {'-' * (20 + 16 * len(all_metrics))}")
        for ds_name, ds_res in res_all_datasets.items():
            row = f"  {ds_name:<20s}"
            for m in all_metrics:
                val = ds_res.get(m, "")
...
if __name__ == "__main__":
    # Normalize torch.distributed.launch arg name to match HfArgumentParser fields.
    sys.argv = [arg.replace("--local-rank", "--local_rank") if arg.startswith("--local-rank") else arg for arg in sys.argv]
