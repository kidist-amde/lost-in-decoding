...
# =============================================================================
# Table 3 reproduction: effectiveness & efficiency with different m and k
# on MSMARCO Dev (+ TREC DL 2019/2020).
#
# Sweeps:  m ∈ {16, 32, 64}  ×  k ∈ {10, 100}
#
# Paper-reported numbers (MSMARCO Dev):
#   m=16,  k=10  → MRR@10 = .342,  Recall@10 = .577
#   m=32,  k=10  → MRR@10 = .367,  Recall@10 = .626
#   m=64,  k=10  → MRR@10 = .379,  Recall@10 = .641
#   m=16,  k=100 → MRR@10 = .355,  Recall@10 = .620
#   m=32,  k=100 → MRR@10 = .372,  Recall@10 = .652
#   m=64,  k=100 → MRR@10 = .385,  Recall@10 = .670
# =============================================================================
...
lex_topk=1000                        # Stage 1 always retrieves top-1000
...
for m in 16 32 64; do
    ...
    lex_out_dir="$model_dir/table3/m_${m}/lex_ret_${lex_topk}"

    # ── Stage 1: Lexical retrieval (simultaneous decoding) ───────────────
    # Single GPU, no torch.distributed.launch
    stage1_marker="$lex_out_dir/MSMARCO/run.json"
    if [ -f "$stage1_marker" ]; then
        ...
        python -m t5_pretrainer.evaluate \
            ...
            2>&1 | tee "$LOG_DIR/stage1_m${m}.log"
    fi
    ...
    for k in 10 100; do
        ...
        # ── Stage 2: Sequential decoding (planning-ahead constrained beam search)
        stage2_marker="$smt_out_dir/MSMARCO/run.json"
        if [ -f "$stage2_marker" ]; then
            ...
            python -m torch.distributed.launch --nproc_per_node=1 -m t5_pretrainer.evaluate \
                ...
                2>&1 | tee "$LOG_DIR/stage2_m${m}_k${k}.log"
        fi
        ...
        python -m t5_pretrainer.evaluate \
            ...
            --q_collection_paths="$q_collection_paths" \
            --eval_qrel_path="$eval_qrel_path" \
            2>&1 | tee "$LOG_DIR/stage3_m${m}_k${k}.log"
...
echo ""
echo "============================================================"
echo " Table 3 Results Summary (MSMARCO Dev)"
echo "============================================================"
echo ""
...
for m in 16 32 64; do
    if [ "$m" -eq 64 ]; then
        lex_dir="$model_dir/table3/m_64/lex_ret_${lex_topk}"
    else
        lex_dir="$model_dir/table3/m_${m}/lex_ret_${lex_topk}"
    fi
    for k in 10 100; do
...
echo ""
echo "Paper-reported values for comparison:"
...
echo ""
echo "Full per-dataset results in: $LOG_DIR/"
echo "Done."
